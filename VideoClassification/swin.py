...
class VideoClassifier(nn.Module):
    def __init__(self, num_classes, dropout):
        ...
        self.backbone = swin3d_b(pretrained=True)
        
        if isinstance(self.backbone.head, nn.Linear):
            in_features = self.backbone.head.in_features
        else:
            raise ValueError("Unexpected structure of swin3d_b head")
...
model = VideoClassifier(num_classes=7, dropout=0.2).to(device)
...
plt.savefig('test/reports/training_loss_swin.png')
...
with open('test/reports/classification_report_swin.txt', 'w') as f:
    f.write(report)
